...
export class ComponentGenerator {
    ...
    generateAttributes(): boolean {
        const allAttrs = new Map<string, { type: string; nullable: boolean }>();
        ...
        for (const comp of this.#components) {
            ...
            for (const attr of comp.mandatoryAttrs) {
                ...
                const type = this.#attrType(attr);
                allAttrs.set(key, { type, nullable: attr.nullable });
                mandatory.push(key);
            }
        ...
        this.file.components.undefine("Attributes");
        const intf = this.file.components.statements("export interface Attributes {", "}");
        intf.shouldDelimit = false;
        for (const [key, { type }] of allAttrs) {
            intf.atom(`${key}: ${type}`);
        }

        // Generate the Components tuple inside a namespace
        const ns = this.file.components.statements("export namespace Attributes {", "}");
    ...
    generateCommands(): boolean {
        ...
        this.file.components.undefine("Commands");
        const extendsClause = interfaceNames.map(n => `Commands.${n}`).join(", ");
        this.file.components.atom(`export interface Commands extends ${extendsClause} {}`);

        // Generate Commands namespace with per-component interfaces and Components tuple
        const ns = this.file.components.statements("export namespace Commands {", "}");
    ...
    generateEvents(): boolean {
        ...
        for (const comp of this.#components) {
            ...
            for (const evt of comp.mandatoryEvents) {
                ...
                const type = this.#eventPayloadType(evt);
                allEvents.set(key, type);
                mandatory.push(key);
            }
        ...
        this.file.components.undefine("Events");
        const intf = this.file.components.statements("export interface Events {", "}");
        intf.shouldDelimit = false;
        for (const [key, type] of allEvents) {
            intf.atom(`${key}: ${type}`);
        }

        // Generate the Components tuple inside a namespace
        const ns = this.file.components.statements("export namespace Events {", "}");
        const tuple = ns.expressions("export type Components = [", "]");
        for (const entry of componentEntries) {
            this.#emitFlagEntries(tuple, entry.condition, obj => {
                if (entry.mandatory.length) {
    ...
    generateFeatures() {
        ...
        this.file.components.atom(`export type Features = ${names.join(" | ")}`);
    }
