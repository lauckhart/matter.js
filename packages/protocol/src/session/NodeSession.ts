...
export class NodeSession extends SecureSession {
    ...
    get name() {
        return `${this.isPase ? "pase" : "case"}/${this.#id}`;
    }
    ...
    override async initiateClose(shutdownLogic?: () => Promise<boolean | void>) {
        await super.initiateClose(async () => {
            ...
            if (defer || this.hasActiveExchanges) {
                logger.debug("Session", Diagnostic.strong(this.name), "will end when exchanges end");
    ...
    protected override async close() {
        if (!this.#isPeerLost) {
            try {
                ...
                logger.error(`Unhandled error in ${this.name} graceful close handler:`, e);
            }
        }
