...
import { camelize, Diagnostic, Logger, UnexpectedDataError } from "@matter/general";
...
export function normalizeAttributeStatus(
    ...
    data.forEach(entry => {
        ...
        result.push({
            path: { nodeId, endpointId, clusterId, attributeId, attributeName: camelize(attributeModel.name) },
            status: status.status,
            clusterStatus: status.clusterStatus,
        });
    });
...
export function normalizeAndDecodeAttributeData(
    ...
    responseList.forEach(values => {
        ...
        try {
            ...
            const attributeName = camelize(attributeModel.name);
            let schema;
            try {
