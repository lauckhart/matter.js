...
export class Conformance extends Aspect<Conformance.Definition> {
    ...
    declare extend: (other: Conformance) => Conformance;
}
...
function ParsedAst(conformance: Conformance, definition: string) {
    ...
    const tokens = Tokenizer(conformance, definition);
    return parseGroup();

    function parseGroup(end?: Tokenizer.Operator): Conformance.Ast {
        ...
        while (true) {
            ...
            if (optional) {
                ...
                expr = parseChoice(expr) as Conformance.Ast;
                group.push(expr);
            } else {
    ...
    function atOperator(operator: Tokenizer.Operator) {
        const { token } = tokens;
        return token && token.type === operator;
    }

    function parseExpression() {
        const elements = [] as (Tokenizer.Operator | Conformance.Ast | string)[];
        ...
        Parser.BinaryOperatorPrecedence.forEach(operators => {
            for (let i = 0; i < elements.length; i++) {
                if (operators.indexOf(elements[i + 1] as Tokenizer.Operator) !== -1) {
    ...
    function choiceName(): Conformance.ChoiceName | "?" {
        ...
        return value as Conformance.ChoiceName;
    }

    function parseChoice(expr: string | Conformance.Ast | undefined): string | Conformance.Ast | undefined {
        if (!atOperator(".")) {
            return expr;
        }
        ...
        if (name === "?") {
            conformance.error("INVALID_CHOICE", 'Choice indicator (".") must be followed by a single lowercase letter');
        }
        const choice = {
            name: name,
            expr: expr,
        ...
        if (tokens.token?.type === "value") {
            choice.num = tokens.token.value as number;
            tokens.next();
        }
        if (atOperator("+")) {
            choice.orMore = true;
    ...
    function parseAtomicExpressionWithoutChoice(): string | Conformance.Ast | undefined {
        if (!tokens.token) {
