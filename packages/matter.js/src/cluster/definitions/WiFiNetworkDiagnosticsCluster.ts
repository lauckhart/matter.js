...
/**
 * Standard WiFiNetworkDiagnostics cluster properties.
 *
 * @see {@link MatterCoreSpecificationV1_1} § 11.14
 */
export const WiFiNetworkDiagnosticsMetadata = ClusterMetadata({
    id: 0x36,
...
export const BaseComponent = ClusterComponent({
    attributes: {
         ...
         */
        wiFiVersion: Attribute(2, TlvNullable(TlvEnum<TlvWiFiVersionEnum>()), { default: null, readAcl: AccessLevel.View }),
...
export type WiFiNetworkDiagnosticsCluster<T extends TypeFromPartialBitSchema<typeof WiFiNetworkDiagnosticsMetadata.features>> = 
    typeof WiFiNetworkDiagnosticsMetadata
    & { supportedFeatures: T }
    & typeof BaseComponent
    & (T extends { errorCounts: true } ? typeof ErrorCountsComponent : {})
    & (T extends { packetCounts: true } ? typeof PacketCountsComponent : {});

export function WiFiNetworkDiagnosticsCluster<T extends (keyof typeof WiFiNetworkDiagnosticsMetadata.features)[]>(...features: [ ...T ]) {
    const cluster = {
        ...WiFiNetworkDiagnosticsMetadata,
        supportedFeatures: BitFlags(WiFiNetworkDiagnosticsMetadata.features, ...features),
        ...BaseComponent
    };
    extendCluster(cluster, ErrorCountsComponent, { errorCounts: true });
    extendCluster(cluster, PacketCountsComponent, { packetCounts: true });
    
    return cluster as unknown as WiFiNetworkDiagnosticsCluster<BitFlags<typeof WiFiNetworkDiagnosticsMetadata.features, T>>;
};