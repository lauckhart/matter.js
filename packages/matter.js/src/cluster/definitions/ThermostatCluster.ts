...
import { GlobalAttributes, Attribute, OptionalAttribute, OptionalWritableAttribute, AccessLevel, WritableAttribute, Command, TlvNoResponse, OptionalCommand, OptionalFixedAttribute, FixedAttribute, Cluster } from "../../cluster/Cluster.js";
...
export function ThermostatCluster<T extends ThermostatCluster.Feature[]>(...features: [...T]) {
    ...
    extendCluster(cluster, ThermostatCluster.NotLocalTemperatureNotExposedComponent, { localTemperatureNotExposed: false });
    extendCluster(cluster, ThermostatCluster.AutoModeComponent, { autoMode: true });
    extendCluster(cluster, ThermostatCluster.ScheduleConfigurationComponent, { scheduleConfiguration: true });
    extendCluster(cluster, ThermostatCluster.SetbackComponent, { setback: true });
...
export namespace ThermostatCluster {
    ...
    export type Type<T extends TypeFromPartialBitSchema<typeof Metadata.features>> =
        ...
        & (T extends { autoMode: true, heating: false } ? never : {})
        & (T extends { autoMode: true, cooling: false } ? never : {})
        & (T extends { heating: false, cooling: false } ? never : {});

    /**
     * Thermostat cluster metadata.
     *
     * @see {@link MatterApplicationClusterSpecificationV1_1} § 4.3
     */
