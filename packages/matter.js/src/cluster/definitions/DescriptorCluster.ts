...
/**
 * Standard Descriptor cluster properties.
 *
 * @see {@link MatterCoreSpecificationV1_1} § 9.5
 */
export const DescriptorMetadata = ClusterMetadata({ id: 0x1d, name: "Descriptor", revision: 1 });
...
export type DescriptorCluster<T extends TypeFromPartialBitSchema<typeof DescriptorMetadata.features>> = 
    typeof DescriptorMetadata
    & { supportedFeatures: T }
    & typeof BaseComponent;

export function DescriptorCluster<T extends (keyof typeof DescriptorMetadata.features)[]>(...features: [ ...T ]) {
    const cluster = {
        ...DescriptorMetadata,
        supportedFeatures: BitFlags(DescriptorMetadata.features, ...features),
        ...BaseComponent
    };
    
    return cluster as unknown as DescriptorCluster<BitFlags<typeof DescriptorMetadata.features, T>>;
};