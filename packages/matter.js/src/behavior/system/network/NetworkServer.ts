...
export class NetworkServer extends NetworkBehavior {
    ...
    #configureListeners(config: unknown[]) {
        ...
        for (const addr of config) {
            ...
            const { protocol, address, hostname, port } = addr as Record<string, any>;
            ...
            switch (protocol) {
                ...
                case undefined:
                    hasUdp = true;
                    listen.push({ protocol: "udp", address: hostname, port });
                    break;
...
export namespace NetworkServer {
    ...
    export class State extends NetworkBehavior.State {
         ...
         *
         *   * `network.listen.protocol` either "ble" or "udp" (default is "udp")
