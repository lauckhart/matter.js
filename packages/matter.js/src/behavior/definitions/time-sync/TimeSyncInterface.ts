...
export namespace TimeSyncInterface {
     ...
     * command, if the stated Granularity and TimeSource are acceptable. The server shall update its UTCTime attribute
     ...
     *
     * server does not plan to maintain time). It shall also update its TimeSource attribute to Admin. It shall also
     * update its last known good UTC time.
     *
     * If the server updates its UTCTime attribute, it shall accept the command with a status code of SUCCESS. If it
     * opts to not update its time, it shall fail the command with a cluster specific Status Code of TimeNotAccepted.
     *
