...
export namespace NetworkCommissioningInterface {
     ...
     * associated network interface that are possible to join, such as all visible Wi-Fi access points for Wi-Fi
     * cluster server instances, all Thread PANs for Thread cluster server instances, within bounds of the maximum
     * response size.
     *
     * Scanning for a specific network (i.e. directed scanning) takes place if a network identifier (e.g. Wi-Fi SSID)
     * is provided in the command arguments. Directed scanning shall restrict the result set to the specified network
     * only.
     *
     ...
     * response due to changes of networking interface configuration at runtime for the interface over which the
     * command was invoked, or if it is currently unable to proceed with such an operation.
     *
     ...
     * configuration with the same NetworkID. The relative order of the entries in the Networks attribute list shall
     ...
     * proceed with such an operation, such as if it is currently attempting to connect in the background, or is
     ...
     *
     * of configuration if the CommissioningComplete command (see Section 11.9.6.6, “CommissioningComplete Command”) is
     ...
     * method to determine success of the operation is operational discovery of the Node on the new operational
     ...
     * attribute, the command shall immediately respond with NetworkConfigResponse having NetworkingStatus status field
     * set to OutOfRange.
     *
     * On success, the NetworkConfigResponse command shall have its NetworkIndex field set to the 0- based index of the
     * entry in the Networks attribute that was just updated, matching the incoming NetworkIndex, and a
     ...
     * moved to allow the insertion, in a way that they all retain their existing relative order between each other,
     * with the exception of the newly re-ordered entry.
     *
     ...
     *
     * In the above outcome, FancyCat and BlueDolphin moved "down" and Home-Guest became the highest priority network
     * in the list.
     *
     * On receiving ReorderNetwork with:
     *
     ...
     * addition/update.
     *
     * This field shall contain the SSID to which to attempt connection. Specific BSSID selection is not supported by
     * this cluster.
     *
