...
export namespace OperationalCredentialsInterface {
     ...
     *
     * recipient shall fail the command with a Status Code of INVALID_COMMAND. The AttestationNonce field shall be used
     * in the computation of the Attestation Information.
     *
     * @see {@link MatterCoreSpecificationV1_1} § 11.17.6.1
     */
    export type AttestationRequestRequest = TypeFromSchema<typeof OperationalCredentials.TlvAttestationRequestRequest>;
     ...
     */
    export type CertificateChainRequestRequest = TypeFromSchema<typeof OperationalCredentials.TlvCertificateChainRequestRequest>;
     ...
     *
     * If the IsForUpdateNOC field is present and set to true, the internal state of the CSR associated keypair shall
     * be tagged as being for a subsequent UpdateNOC, otherwise the internal state of the CSR shall be tagged as being
     * for a subsequent AddNOC. See Section 11.17.6.8, “AddNOC Command” and Section 11.17.6.9, “UpdateNOC Command” for
     ...
     */
    export type CsrRequestRequest = TypeFromSchema<typeof OperationalCredentials.TlvCsrRequestRequest>;
     ...
     * a StatusCode of InvalidNOC as described in Section 11.17.6.7.2, “Handling Errors”:
     *
     ...
     * accessing fabric, then an NOCResponse with a StatusCode of LabelConflict shall be returned for the command and
     * there shall NOT be any permanent changes to any Fabric data.
     *
     ...
     * NOCResponse with a StatusCode of InvalidFabricIndex shall be returned for the command and there shall NOT be any
     * permanent changes to any device data.
     *
     ...
     *      Matter related data on the node which was created since it was commissioned. This includes all
     ...
     * secure sessions, exchanges and interaction model constructs shall also be removed. Since this operation involves
     * the removal of the secure session data that may underpin the current set of exchanges, the Node invoking the
     ...
     * operational certificates, etc. Any remaining Trusted Roots no longer referenced by any operational certificate
     * shall also be removed. All secure sessions, exchanges and interaction model constructs related to the
     * Operational Identity under the given Fabric shall also be removed. Following the removal, an NOCResponse with a
     ...
     * removed. All secure sessions, exchanges and interaction model constructs related to the Operational Identity
     * under the given Fabric shall also be removed. Since this operation involves the removal of the secure session
     * data that may underpin the current set of exchanges, the Node invoking the command SHOULD NOT expect a response
     ...
     * representation, to the TrustedRootCertificates Attribute list and shall ensure the next AddNOC command executed
     * uses the provided certificate as its root of trust.
     *
     ...
     * would cause the new invocation to add a second root certificate within a given fail- safe timer period, then
    ...
    export interface Base {
         ...
         * @see {@link MatterCoreSpecificationV1_1} § 11.17.6.1
         */
        attestationRequest(request: AttestationRequestRequest): MaybePromise<AttestationResponse>;
         ...
         * @see {@link MatterCoreSpecificationV1_1} § 11.17.6.3
         */
        certificateChainRequest(request: CertificateChainRequestRequest): MaybePromise<CertificateChainResponse>;
         ...
         * @see {@link MatterCoreSpecificationV1_1} § 11.17.6.5
         */
        csrRequest(request: CsrRequestRequest): MaybePromise<CsrResponse>;
