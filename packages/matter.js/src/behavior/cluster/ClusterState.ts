 ...
 */
export type ClusterState<C extends ClusterType, B extends Behavior.Type> =
    ClusterState.Endpoint<C, B> & ClusterState.Fabric<C, B>;
...
export namespace ClusterState {
     ...
     */
    export type EndpointProperties<C> =
        EndpointAttributeProperties<ClusterType.AttributesOf<C>>;

    export type EndpointAttributeProperties<A extends Record<string, ClusterType.Attribute>> =
        AttributeProperties<{
            [N in keyof A as A[N] extends { fabricScoped: true } ? never : N ]:
                A[N]
        }>;
     ...
     */
    export type FabricProperties<C> =
        FabricAttributeProperties<ClusterType.AttributesOf<C>>;

    export type FabricAttributeProperties<A extends Record<string, ClusterType.Attribute>> =
        AttributeProperties<{
            [N in keyof A as A[N] extends { fabricScoped: true } ? N : never ]:
                A[N]
        }>;

    export type AttributeProperties<A extends Record<string, ClusterType.Attribute>> =
        & ClusterType.SomeOptional<
            A,
            & {
                [N in keyof A as A[N] extends { fixed: true } ? never : N ]:
                    TypeFromSchema<A[N]["schema"]>
            }
            & {
                readonly [N in keyof A as A[N] extends { fixed: true } ? N : never ]:
                    TypeFromSchema<A[N]["schema"]>
            }
        >;
}
