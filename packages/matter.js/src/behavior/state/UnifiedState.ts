 ...
 */
const cache = new WeakMap<State.Type, WeakMap<State.Type, UnifiedState<any, any>>>();

/**
 * "Unified" state includes all state values (endpoint and fabric) for a
 * behavior.
 */
export type UnifiedState<ES extends State.Type, FS extends State.Type> = InstanceType<ES> & InstanceType<FS>;

/**
 * Create unified state type that delegates to scoped states.
 */
export function UnifiedState<const ES extends State.Type, const FS extends State.Type>(
    EndpointScope: ES,
    FabricScope: FS,
...
export namespace UnifiedState {
    export type Type<ES extends State.Type = State.Type, FS extends State.Type = State.Type> = new (
...
function createDescriptor(
    ...
    if (property === ENDPOINT_SCOPE) {
        ...
        descriptor.get = function (this: Internal) {
            if (this[FABRIC_SCOPE] === undefined) {
                throw new ImplementationError(
                    `Illegal read of property ${behaviorName}.state.${name} outside fabric scope`,
                );
            }
        ...
        descriptor.set = function (this: Internal, value: any) {
            if (this[FABRIC_SCOPE] === undefined) {
                throw new ImplementationError(
                    `Illegal write to property ${behaviorName}.state.${name} outside fabric scope`,
                );
            }
