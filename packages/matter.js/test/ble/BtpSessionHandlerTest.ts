...
describe("BtpSessionHandler", () => {
    describe("Test Handshake", () => {
        ...
        it("Client does not share the same supported BTP version", async () => {
            ...
            await expect(async () => {
                ...
                new BtpProtocolError("No supported BTP version found in 5")
            );
        });
    });
    ...
    describe("Test Matter Message handling", () => {
        ...
        it("Btp packet must not be 0", async () => {
            await expect(
                btpSessionHandler?.sendMatterMessage(ByteArray.fromHex(""))
            ).rejects.toThrow(
                new BtpFlowError("BTP packet must not be empty")
            );
        });
