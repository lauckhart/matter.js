...
const logger = new Logger("Cancelable");
...
export class CancelablePromise<T = void> implements Promise<T>, Cancelable {
    ...
    constructor(
        ...
        this.#promise = new Promise((resolve, reject) => {
            ...
            executor(
                ...
                (reason?: any) => {
                    if (this.#isSettled) {
                        logger.warn(`Cancelable promise rejected after settle:`, reason);
                        return;
                    }
    ...
    static reject(cause: any): CancelablePromise<any> {
        const result = Promise.reject(cause) as CancelablePromise<any>;
        result.cancel = () => {};
        return result;
    }
}
    ...
    implements AsyncIterator<T, TReturn, TNext>, Cancelable
{
    ...
    next(...[value]: [] | [TNext]): Promise<IteratorResult<T, TReturn>> {
        ...
        next.catch(reason => {
            if (this.#settled) {
                logger.warn(`Cancelable async iterator rejected after return:`, reason);
            }
        });
