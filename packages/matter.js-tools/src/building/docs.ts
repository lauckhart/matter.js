...
const PLUGINS = [
    // You can only load this plugin once because it uses an ugly monkey patch and global state.  Fortunately the output
    // is not particularly desirable.  TODO - replace with logic to detect unexported types
    //"typedoc-plugin-missing-exports",
    //
    // Eventual plan is to process raw reflection data directly in browser but for now we just output default HTML
...
export async function buildDocs(pkg: Package, progress: Progress) {
    ...
    // We point typedoc do each of our public exports
    const entryPoints = Array<string>();

    // // Map of stupid names that typedoc chooses to actual export names
    // const moduleNameMapping = {} as Record<string, string>;
    ...
    for (const name in modules) {
        ...
        // const stupidName = relative(basePath, path)
        //     .replace(/.ts$/, "")

        //     // typedoc strips "/index".  our hacks below strip "/export"
        //     .replace(/\/index$/, "");

        // if (name.startsWith(packagePrefix)) {
        //     moduleNameMapping[stupidName] = name.substring(packagePrefix.length);
        // } else if (name === pkg.name) {
        //     // Flags for merge with parent
        //     moduleNameMapping[stupidName] = ".";
        // }
    }
    ...
    const app = await createApplication({
        ...
        // Removes a couple hundred MB of tabs on packages/matter.js so worth uncommenting at some point
        //pretty: false,
    ...
    app.converter.on(Converter.EVENT_CREATE_DECLARATION, (/*_context, reflection: DeclarationReflection*/) => {
        ...
        //     const actualName = moduleNameMapping[reflection.name];
        //     if (actualName === ".") {
        //         // Typedoc chose a module such as "export" for root children of the module
        //         analysis.mergeWithParent.add(reflection);
        //     } else if (actualName !== undefined) {
        //         reflection.name = actualName;
        //     }
        // }
    });
 ...
 *
 * Instead we create a map of typedoc's naive module name to the correct name so we can update names as reflection load.
 */
