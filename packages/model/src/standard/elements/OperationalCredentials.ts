    ...
    Command(
        ...
        Field({
            ...
            details: "This field shall be set to the Vendor ID of the entity issuing the AddNOC command. This value shall " +
                ...
                "the array in implementation-specific ways, as long as they follow the correct format when being read " +
                "from the NOCs list or used within other protocols such as CASE." +
                "\n" +
                "5. The NOCs list shall reflect the incoming NOC from the NOCValue field and ICAC from the ICACValue " +
                "field (if present)." +
                "\n" +
                "6. The operational discovery service record shall immediately reflect the new Operational " +
                "Identifier, such that the Node immediately begins to exist within the Fabric and becomes reachable " +
                "over CASE under the new operational identity." +
                "\n" +
                "7. The receiver shall create and add a new Access Control Entry using the CaseAdminSubject field to " +
                "grant subsequent Administer access to an Administrator member of the new Fabric. It is recommended " +
                "that the Administrator presented in CaseAdminSubject exist within the same entity that is currently " +
                "invoking the AddNOC command, within another of the Fabrics of which it is a member." +
                "\n" +
    ...
    Command(
        {
            ...
            details: "This command shall replace the NOC and optional associated ICAC (if present) scoped under the " +
                ...
                "be revoked and removed, to an outcome equivalent to the disappearance of the prior Node, except for " +
                "the ongoing CASE session context, which shall temporarily remain valid until the NOCResponse has " +
                "been successfully delivered or until the next transport-layer error, so that the response can be " +
                "received by the Administrator invoking the command." +
                "\n" +
                "Thereafter, the Node shall respond with an NOCResponse with a StatusCode of OK and a FabricIndex " +
    ...
    Command(
        ...
        Field({
            ...
            details: "This field shall contain the Fabric Index reference (see fabric-index) associated with the Fabric " +
                ...
                "time synchronization cluster is present on the Node, the TrustedTimeSource and DefaultNtp shall be " +
                "set to null. Any Matter related data including logs, secure sessions, exchanges and interaction " +
                "model constructs shall also be removed. Since this operation involves the removal of the secure " +
                "session data that may underpin the current set of exchanges, the Node invoking the command SHOULD " +
                ...
                "process of irrevocably deleting all associated Fabric-Scoped data, including Access Control Entries, " +
                "Access Restriction Entries, bindings, group keys, operational certificates, etc. Any remaining " +
                "Trusted Roots no longer referenced by any operational certificate shall also be removed. If a time " +
                "synchronization cluster is present on the Node, and the TrustedTimeSource FabricIndex matches the " +
                "given FabricIndex, the TrustedTimeSource shall be set to null. All secure sessions, exchanges and " +
                ...
                "Roots no longer referenced by any operational certificate shall also be removed. If a time " +
                "synchronization cluster is present on the Node, and the TrustedTimeSource FabricIndex matches the " +
                "given FabricIndex, the TrustedTimeSource shall be set to null. All secure sessions, exchanges and " +
                "interaction model constructs related to the Operational Identity under the given Fabric shall also " +
                "be removed. Since this operation involves the removal of the secure session data that may underpin " +
                "the current set of exchanges, the Node invoking the command SHOULD NOT expect a response before " +
