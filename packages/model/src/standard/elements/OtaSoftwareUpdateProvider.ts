        ...
        Field(
            {
                ...
                details: "This field, when present, shall contain a URI where the OTA Requestor SHOULD download a Software " +
                    ...
                    "\n" +
                    "2. The URI’s authority field shall contain only the host portion and shall use string representation " +
                    "of the Operational Node ID of the Node where to proceed with the download, on the same Fabric on " +
                    "which the OTA Requestor received the QueryImageResponse." +
                    "\n" +
                    "3. The encoding of the Node ID in the host field shall use an uppercase hexadecimal format, using " +
                    "exactly 16 characters to encode the network byte order value of the NodeID, in a similar fashion as " +
                    "the Node Identifier portion of the Operational Instance Name." +
                    "\n" +
                    "a. The Operational Node ID in the host field shall match the NodeID of the OTA Provider responding " +
                    "with the QueryImageResponse. The usage of a different Node ID than that of the provider is reserved " +
                    "for future use. This constraint reduces the number of independent CASE secure channel sessions that " +
                    "have to be maintained to proceed with OTA software updates, thus reducing energy and resource " +
                    "utilization for the software update process." +
                    "\n" +
                    ...
                    "\n" +
                    "8. The path field shall employ absolute path representation and shall contain the file designator of " +
                    "the software image to download at the BDX server. When used with the BDX server, the leading / " +
                    "separating the URI authority from the path shall be omitted. When contacting the BDX server, further " +
                    "processing of the file designator shall NOT be done, including handling of URL-encoded escape " +
                    "sequences. Rather, the exact octets of the path, as received shall be the values used by both client " +
                    "and server in handling the file designator." +
                    "\n" +
