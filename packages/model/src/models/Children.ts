 ...
 */
export interface Children<T extends Model = Model> extends Array<Model.ChildOf<T>> {
    /**
     * Add a child.
     */
    add(child: Children.DefinitionOf<T>): Model;
     ...
     */
    push(...children: Children.DefinitionOf<T>[]): number;
     ...
     */
    splice(index: number, deleteCount?: number, ...toAdd: Children.DefinitionOf<T>[]): Model.ChildOf<T>[];
...
export function Children<T extends Model = Model>(
    initial: Iterable<Children.DefinitionOf<T>>,
    ...
    const children = Array<Children.DefinitionOf<T>>();
    ...
    function reify() {
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            child = Model.create(child) as Model.ChildOf<T>;
            adopt(child);
            children[i] = child;
        }
        reified = true;
     ...
     * after insertion.
     */
    function insertionFormOf(child: Children.DefinitionOf<T>) {
        if (child instanceof Model) {
            return child;
        }

        if (reified || hasModel(child)) {
            return Model.create(child) as Model.ChildOf<T>;
        }
    ...
    function splice(index: number, deleteCount?: number, ...toAdd: Children.DefinitionOf<T>[]) {
    ...
    const self = new Proxy(children, {
        ...
        deleteProperty: (_target, p) => {
            let child: undefined | Children.DefinitionOf<T>;
...
export namespace Children {
    /**
     * The input type for children.  Children may be added as elements or models.
     */
    export type DefinitionOf<T extends Model> = Model.ElementOf<Model.ChildOf<T>> | Model.ChildOf<T>;
